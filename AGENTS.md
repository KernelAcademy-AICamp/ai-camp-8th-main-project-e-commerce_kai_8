...
2026-08-17 검색 업그레이드 작업을 돌아보며 정한 규칙이다. 그때 **측정 장치가 측정 대상의 2배**가 되고(하네스 1,258줄 vs 기능 619줄), 스스로 만든 평가 규칙을 스스로 어기고, 이론적 위험을 고치다 실제 기능을 깨뜨렸다. 근거와 상세는 [외부 조사](docs/atee/foundation/llm-search-references.md).

### 만들기 전에 15분 찾아본다

이번에 평가 하네스와 배포 방식을 조사 없이 직접 짰는데 **둘 다 표준 도구가 있었다**(promptfoo, GitHub Scientist). 반대로 임베딩 모델은 제대로 조사하고 실측해 골랐다. 차이는 "모델 선택"으로 봤는지 "당연히 짜야 하는 부속"으로 봤는지다.

→ **결정처럼 보이지 않는 부속일수록 먼저 찾아본다.** 찾은 결과는 채택하든 기각하든 근거와 함께 문서에 남긴다.

### 작업 단위

- **스펙은 100~150줄. 한 스펙 = 한 덩어리.** 여러 하위 시스템을 한 스펙에 담으면 리뷰가 매번 전체를 대상으로 돌아 Blocker가 끝나지 않는다.
- **계획 하나 = 배포 가능한 한 조각.** `phase1`/`phase2`/`shadow1`처럼 이름으로 나눈다.
- **새 경로는 shadow로 먼저.** 응답에 영향 0인 상태로 돌려 기록만 비교하고, 일치가 확인되면 전환한다. 바로 켜면 "평가와 실제 동작이 다른" 상태가 생긴다.
- **한 번에 한 변수만 바꾼다.** 여러 개를 함께 바꾸면 무엇이 효과였는지 못 가른다.
...
- **① 지금 해를 끼침** — 즉시 고친다.
- **② 나중에 비싸짐** (재실행 불가·계약 파손·데이터 오염) — 이번에 고친다.
- **③ 이론적 위험** — 기록만 하고 **처리 여부는 사람이 정한다.**

사용자 0명 단계에서 질의 문법 주입(③)을 ①처럼 다뤄 핵심 기능을 깨뜨렸다. **등급은 AI가 매기고 ②③의 처리는 사람이 결정한다.**

### 측정은 제품 단계에 맞춘다

판단에 필요한 정밀도를 넘는 측정은 비용이다. 검증 전 단계에서는 **0건율 + 눈 검증**이면 대개 충분하고, 정교한 지표는 A/B를 진짜 비교해야 할 때 꺼낸다. **어느 정밀도가 필요한지는 제품 단계가 정한다 — AI가 정할 일이 아니다.**

### 교차 리뷰는 유지한다

Codex 교차 리뷰가 실제 결함을 계속 잡았다(마이그레이션 재실행 불가, 삭제 약속 파손, 기준선 43.4%→27.8% 과대계산). **다만 조각마다 한 번**이지, 전체를 여러 번 도는 게 아니다.
