 ...
 * - **Cohort size** = 그 Day를 물어볼 수 있는 계정 수. Mixpanel 리텐션 리포트의
 *   `Size` 열에 해당한다.
 ...
 *
 * **Cohort size가 Day마다 다르다.** Day N을 물으려면 그 계정이 N일 전에
 * 시작했어야 한다. 오늘 처음 온 계정은 Day 1 분모에 들어갈 수 없다 — 아직
 ...
 *    **이은 적 없는 기기는 빠진다.** 폰과 노트북으로 본 것은 한 계정으로 합쳐진다.
 *    예전 기기 단위 판에서는 **실제 사람의 retention보다
...
export const returnCurve: MetricDefinition = {
  id: "return-curve",
  title: "재방문 곡선 · N-Day Retention (계정 단위)",
  ...
  sql: `
    ...
    -- 코호트 진입일 = Day 0
    첫날 as (
      select account_id, min(날짜) as 시작일 from 활동일 group by 1
    ),
    ...
    select
      ...
      count(*) filter (
        where o.지난일수 >= d.n
          and exists (
            select 1 from 활동일 a
            where a.account_id = o.account_id and a.날짜 = o.시작일 + d.n
          )
      )::int                                           as "Retained",
      round(
        100.0 * count(*) filter (
