...
| 무엇             | 주소                                                                                |
| ---------------- | ----------------------------------------------------------------------------------- |
| `develop` 프리뷰 | https://ai-camp-8th-main-project-e-commerce-kai-8-git-develop-er-yuk-sa.vercel.app/ |

**커밋마다 생기는 주소(해시가 붙은 것) 말고 위 `-git-develop-` 별칭을 쓴다.** 배포할 때마다 바뀌지 않는다.
...
| 환경변수           | 위 3개                                                                   |
| Ignored Build Step | `admin/`이 바뀔 때만 빌드 — [vercel.json](vercel.json)의 `ignoreCommand` |

**`admin/`이 안 바뀌면 빌드하지 않는다.** [vercel.json](vercel.json)의 `ignoreCommand`가 직전
커밋과 비교해 `admin/`에 변경이 없으면 빌드를 건너뛴다. 이게 없으면 `frontend`만 고쳐도 admin이
매번 빌드되어 무료 한도를 갉아먹는다 — 실측(2026-08-25)으로 `develop` 최근 커밋 20개 중
`admin/`을 건드린 것은 **3개**였다. 나머지 17번은 헛빌드였다.

**대시보드가 아니라 파일에 두는 이유는 리전과 같다.** 설정이 코드에 없으면 다음 사람이 왜 배포가
안 도는지 찾을 방법이 없다. 건너뛴 배포는 목록에 `Canceled`로 남는다 — 실패가 아니라 **안 돌렸다**는
표시다. `HEAD^`가 없어 명령이 실패하면 **빌드하는 쪽**으로 떨어지므로, 틀려도 안전한 방향이다.
...
|                                   | 왜                                                                                                    |
| --------------------------------- | ----------------------------------------------------------------------------------------------------- |
| **잇기 전의 기록**                | 계정 단위 카드는 기기-계정 쌍을 거친다(O-43). 쌍이 생기기 전 기록 중 근거로 되살리지 못한 것은 빠진다 |
