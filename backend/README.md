...
- **`c_search_text`(검색용 물질화, 2026-08-16)는 `c_goods` 재수집이나 `c_thumb_dims.card_ok` 재분류 후 반드시 재생성해야 한다** — `20260816240000_c_search_page.sql` 재실행(테이블 drop 후 재적재)으로 충분. 재생성하지 않으면 자격을 잃은 상품이 검색에 다시 노출되거나 신규 상품이 검색에서 빠진다. 위의 "가격·판매 상태 전수 재수집"과 card_ok 재분류 작업 절차에 이 단계를 포함할 것.
