...
- ⚠️ **`20260817200000`을 다시 돌렸으면 그 뒤 번호 파일도 이어서 돌린다.** 그 파일의 `c_search_page_v2`는 뒤 번호에서 만드는 파서(`c_search_color_parse`·`c_search_price_parse`)를 부른다. PL/pgSQL은 이름을 늦게 풀기 때문에 전부 적용하면 정상이지만, **그 사이 구간에는 anon에 열린 검색 RPC가 없는 함수를 부른다.** 순서:

  ```
  20260817150000 → 20260817200000 → 20260817600000 → 20260817700000
                 → 20260817800000 → 20260817900000 → 20260818000000
  ```
...
재생성 대상이 **다섯**이다. 하나만 돌리면 나머지가 낡는다. **순서가 있다** — `c_search_vocab`은 `c_search_docs`에서 파생되므로 뒤에 돌린다.
...
| 5 | **`c_gender_label_flags`** (2026-08-22) | **`select c_gender_label_flags_rebuild();`** | 성별 라벨 의심 제외 — 피드·믹스·유사·검색 v1·v2 **다섯 함수 전부**가 읽는다 |

> **5번은 마이그레이션 재실행이 아니라 함수 호출 하나다.** `c_goods`(제목·성별)와
> `c_search_fit_measures`(어깨·총장)에서 파생하므로 **그 둘이 갱신된 뒤에** 돌린다.
> 그림자 표를 완성한 뒤 원자 교체하므로 도중에 읽는 쪽이 빈 표를 보지 않는다.
> 안 돌리면 **신규 상품의 잘못된 성별 라벨이 그대로 노출되고**, 이미 고친 상품이
> 낡은 판정에 묶인다. 판정 규칙(낱말·어깨·총장 상한)은 `c_gender_label_ruleset()`
> 한 곳에 있다 — 임계값을 바꾸면 그 함수를 고치고 rebuild를 다시 돌린다.
...
> "3만원 이하"로 통과한 상품이 응답에서는 3만원을 넘는다 — 검색이 낡는 정도가 아니라
> 기준서 G4의 하드 조건을 직접 깨뜨린다. `test_search_fallback.py`의
> `test_search_docs_price_matches_the_catalog`가 이 어긋남을 감시한다(실 DB 필요).

`c_search_vocab`은 오타 교정 사전이다(현재 **3,989개**). 구성은 두 갈래의 합집합 — ⓐ 브랜드명 **전체** 중 3음절 이상, ⓑ 여러 단어 브랜드의 **어절** 중 4음절 이상. ⓑ가 있어야 `무신사 스탠다그`처럼 한 단어만 틀린 오타를 고친다. 제목 빈출어까지 넣었다가 되돌렸다 — 사전 없이는 오타와 "우리가 안 파는 진짜 단어"를 가를 수 없어 `샌들 슬리퍼`가 티셔츠를 반환했다. **낡으면 새 브랜드의 오타가 안 고쳐지고, 사라진 브랜드로 잘못 고친다.** shadow 교체 방식이라 재실행이 안전하고 10초 내에 끝난다.
...
> ✅ **낡으면 알려는 준다** (2026-08-22). `.github/workflows/search-drift.yml`이 하루 1회
> `test_search_fallback.py`를 운영 카탈로그로 돌려 위 네 표의 낡음을 감시하고, 깨지면
> Slack으로 알린다. **재생성을 대신하지는 않는다** — 알림은 사람이 위 절차를 돌리라는 신호다.
> 시크릿 `SUPABASE_DB_URL`이 있어야 실제로 돈다(없으면 건너뛰되 경고를 남긴다).
