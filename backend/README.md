...
> ⚠️ **아직 자동화되지 않았다.** 재수집 파이프라인이 이 두 마이그레이션을 자동으로 부르지 않으므로 **사람이 절차에 넣어야 한다.** 자동 동기화(트리거 또는 재수집 스크립트 연결)는 미구현이며, 그 전까지는 재수집 때마다 수동 실행이 유일한 보장이다. 위의 "가격·판매 상태 전수 재수집"과 card_ok 재분류 작업 절차에 이 단계를 포함할 것.
