...
Orbit이 쓰는 티셔츠 카탈로그. `musinsa-c-db-handoff` 꾸러미(2026-08-12)를 이관한 것으로, 판매자 개인정보가 처음부터 들어오지 않는 `c_*` 계열 수집·적재 파이프라인이다.
...
- 수집 계획 문서의 미정 항목 6개
- `c_goods`를 앱에 노출할 읽기 전용 뷰 설계 (Orbit 피드 요구사항 확정 후)
