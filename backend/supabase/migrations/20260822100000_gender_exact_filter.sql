-- 설정 성별 토글 — 서버 계약 (계획 1단계)
--
-- 계획: docs/plans/2026-08-21-gender-setting-toggle.md
-- 설계: docs/superpowers/specs/2026-08-21-gender-setting-toggle-design.md
--
-- 무엇이 바뀌나
--   1) 성별 조건이 "해당 성별 또는 공용"에서 **등식**이 된다 — 공용(4.1만)과 미상(1,911)이 빠진다.
--   2) 성별 인자가 **기본값 없이 필수**가 되고, 누락·널·허용 밖 값은 **오류로 거부**한다.
--      기본값을 남기면 성별을 안 싣는 옛 요청이 그대로 매칭돼 필터가 조용히 꺼진다.
--      기본값 인자 뒤에 필수 인자를 둘 수 없어 **앞선 인자들의 기본값도 함께 제거**한다.
--      (클라이언트는 이미 모든 인자를 명시해 보내므로 호출부는 그대로다.)
--   3) 믹스의 벡터 후보 배수를 17 → **10**으로 내린다 (0단계 ⑨ 결과 3 실측).
--
...
--
-- 함수 설정 보존 대상 (2026-08-22 실측)
--   · 믹스   search_path=public, extensions | work_mem=16MB
--   · 유사   search_path=public, extensions | work_mem=64MB   ← ALTER FUNCTION으로만 걸려 있었다
--   · 피드   search_path=public
--   · 검색   search_path=public, extensions, pg_temp
--
...
-- ③ 유사 — 성별 인자를 새로 추가하므로 기존 시그니처를 지우고 다시 만든다.
drop function if exists c_similar_page(bigint, int);
...
-- ⑤ 검색 v2 (opt-in) — 성별 인자 추가. 켜져 있지 않아도 함께 바꿔 두어야
--    나중에 켤 때 성별이 조용히 빠지지 않는다.
drop function if exists c_search_page_v2(text, real, bigint, int, text[], text[]);
...
-- ── 소유자·권한 ────────────────────────────────────────────────────────────
-- 지우고 다시 만든 셋은 권한이 사라졌다. 덮어쓴 둘도 명시해 상태를 한곳에서 읽게 한다.
alter function c_feed_page(bigint, bigint, int, text) owner to postgres;
alter function c_mix_page(jsonb, jsonb, bigint[], bigint, int, boolean, text) owner to postgres;
alter function c_similar_page(bigint, int, text) owner to postgres;
alter function c_search_page(text, bigint, int, text) owner to postgres;
alter function c_search_page_v2(text, real, bigint, int, text[], text[], text) owner to postgres;

-- revoke는 역할을 명시한다 — Supabase에서 `from public`만으로는 anon/authenticated가 남는다.
revoke all on function c_feed_page(bigint, bigint, int, text) from public, anon, authenticated;
revoke all on function c_mix_page(jsonb, jsonb, bigint[], bigint, int, boolean, text) from public, anon, authenticated;
revoke all on function c_similar_page(bigint, int, text) from public, anon, authenticated;
revoke all on function c_search_page(text, bigint, int, text) from public, anon, authenticated;
revoke all on function c_search_page_v2(text, real, bigint, int, text[], text[], text) from public, anon, authenticated;
...
grant execute on function c_similar_page(bigint, int, text) to anon, authenticated;
grant execute on function c_search_page(text, bigint, int, text) to anon, authenticated;
grant execute on function c_search_page_v2(text, real, bigint, int, text[], text[], text) to anon, authenticated;
