-- 성별 라벨 의심 상품을 다섯 함수에서 제외한다 (2026-08-22)
--
-- 무엇을: c_gender_label_flags에 담긴 상품(848개)을 피드·믹스·유사·검색 v1·v2에서 뺀다.
--         규칙과 근거는 20260822500000_gender_label_flags.sql 머리주석에 있다.
--
-- 어디에 넣었나 — **성별 등식과 같은 자리**, 언제나 `limit` 앞이다.
--   c_mix_page      벡터 lateral 2곳 + 후보풀 1곳 (한쪽만 막으면 다른 쪽으로 샌다)
--   c_feed_page     후보 선택 절
--   c_similar_page  ANN 후보 절
--   c_search_page   후보 subquery (limit 앞)
--   c_search_page_v2 AND 경로 + 일반 경로 + 하드조건 fallback (세 분기 전부)
--
-- **클라이언트 제외 목록(p_exclude)으로 처리하지 않는다.** 600개 상한과 클라이언트
-- 상태에 의존하면 하드 필터가 아니고 새 요청에서 다시 샌다.
--
-- 보관함은 건드리지 않는다 — 이미 찜한 상품을 조용히 숨기는 것은 탐색 결과 필터와
-- 의미가 다르다. 별도 설계가 필요하다.
--
-- 정본은 **적용 직전 운영 DB에서 뜬 정의**다. 옛 마이그레이션을 복사하면 후보풀
-- 커서(20260822300000)와 앵커 회전(20260822400000)이 되돌아간다.
--
-- 되돌리기: backend/supabase/rollback/20260822600000_gender_label_filter.down.sql
...
begin
  ...
  with page as (
    ...
    where d.width > 0
      ...
      -- 라벨 의심 제외. **limit 앞**이어야 페이지가 짧아지지 않는다.
      and not exists (select 1 from c_gender_label_flags lf where lf.goods_no = d.goods_no)
      and (p_after is null
...
$function$
;
alter function c_feed_page(bigint, bigint, integer, text) owner to postgres;
revoke all on function c_feed_page(bigint, bigint, integer, text) from public, anon, authenticated;
grant execute on function c_feed_page(bigint, bigint, integer, text) to anon, authenticated;
  ...
  for v_try in 0 .. 2 loop
    ...
    from (
      ...
      where (p_after is null or s.goods_no > p_after)
        ...
        -- 라벨 의심 제외. 성별 조건과 같은 자리(limit 앞)여야 한다.
        and not exists (select 1 from c_gender_label_flags lf where lf.goods_no = s.goods_no)
      order by s.goods_no
      limit v_size
    ) page
...
$function$
;
alter function c_search_page(text, bigint, integer, text) owner to postgres;
revoke all on function c_search_page(text, bigint, integer, text) from public, anon, authenticated;
grant execute on function c_search_page(text, bigint, integer, text) to anon, authenticated;
...
$function$
;
alter function c_search_page_v2(text, real, bigint, integer, text[], text[], text) owner to postgres;
revoke all on function c_search_page_v2(text, real, bigint, integer, text[], text[], text) from public, anon, authenticated;
grant execute on function c_search_page_v2(text, real, bigint, integer, text[], text[], text) to anon, authenticated;
