-- 되돌리기 — 20260822100000_gender_exact_filter.sql
--
-- 아래 정의는 **2026-08-22 적용 직전 프로덕션의 현재 정의를 pg_get_functiondef로 뜬 것**이다.
-- 손으로 옮기지 않았다. 이것을 실행하면 성별 등식 이전 상태(성별+공용, 기본값 있는 인자,
-- 후보 배수 17)로 돌아간다.
--
-- ⚠️ 되돌리면 성별 필터의 의미가 "해당 성별 또는 공용"으로 돌아갈 뿐, 성별 인자를 필수로
--    만들기 전의 클라이언트 계약까지 저절로 맞는 것은 아니다. 클라이언트도 함께 되돌린다.
--
...
-- ── 무작위 피드 ──
CREATE OR REPLACE FUNCTION public.c_feed_page(p_seed bigint, p_after bigint DEFAULT NULL::bigint, p_size integer DEFAULT 30, p_gender text DEFAULT NULL::text)
...
-- ── 개인화 믹스 ──
CREATE OR REPLACE FUNCTION public.c_mix_page(p_session jsonb DEFAULT '[]'::jsonb, p_long jsonb DEFAULT '[]'::jsonb, p_exclude bigint[] DEFAULT '{}'::bigint[], p_seed bigint DEFAULT 0, p_size integer DEFAULT 30, p_boost boolean DEFAULT false, p_gender text DEFAULT NULL::text)
...
-- ── 유사 ──
CREATE OR REPLACE FUNCTION public.c_similar_page(p_goods bigint, p_size integer DEFAULT 30)
...
-- ── 검색 v1 ──
CREATE OR REPLACE FUNCTION public.c_search_page(p_query text, p_after bigint DEFAULT NULL::bigint, p_size integer DEFAULT 30)
...
-- ── 검색 v2 ──
CREATE OR REPLACE FUNCTION public.c_search_page_v2(p_query text, p_after_score real DEFAULT NULL::real, p_after bigint DEFAULT NULL::bigint, p_size integer DEFAULT 30, p_exclude text[] DEFAULT NULL::text[], p_exclude_colors text[] DEFAULT NULL::text[])
...
-- ── 소유자·권한 (지우고 다시 만든 셋은 반드시 다시 부여한다) ──
alter function c_similar_page(bigint, int) owner to postgres;
alter function c_search_page(text, bigint, int) owner to postgres;
alter function c_search_page_v2(text, real, bigint, int, text[], text[]) owner to postgres;

revoke all on function c_similar_page(bigint, int) from public, anon, authenticated;
revoke all on function c_search_page(text, bigint, int) from public, anon, authenticated;
revoke all on function c_search_page_v2(text, real, bigint, int, text[], text[]) from public, anon, authenticated;
