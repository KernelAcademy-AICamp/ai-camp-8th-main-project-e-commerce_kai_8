-- 되돌리기: 앵커 회전 (20260822400000)
--
-- 회전 인자를 없애고 20260822300000(후보풀 커서) 시점 정의로 되돌린다.
-- 클라이언트가 p_rotation을 보내는 상태에서 이것만 돌리면 인자가 안 맞아 실패한다 —
-- **프론트를 먼저 되돌리거나 함께 되돌려야 한다.**
begin;

drop function if exists c_mix_page(jsonb, jsonb, bigint[], bigint, integer, boolean, text, bigint, bigint, integer);
