-- 되돌리기: 개인화 믹스 후보풀 커서 (20260822300000)
--
-- 커서 인자·반환 열을 없애고 20260822100000(성별 등식) 시점의 정의로 되돌린다.
-- 클라이언트가 커서를 보내는 상태에서 이걸 돌리면 인자 개수가 안 맞아 요청이 실패한다 —
-- **프론트를 먼저 되돌리거나 함께 되돌려야 한다.**
begin;
...
alter function c_mix_page(jsonb, jsonb, bigint[], bigint, integer, boolean, text) owner to postgres;
revoke all on function c_mix_page(jsonb, jsonb, bigint[], bigint, integer, boolean, text) from public, anon, authenticated;
grant execute on function c_mix_page(jsonb, jsonb, bigint[], bigint, integer, boolean, text) to anon, authenticated;
