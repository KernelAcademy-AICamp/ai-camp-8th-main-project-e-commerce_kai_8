    ...
    >
      {thumbs.length > 2 && (
        <Image
          ...
          fill
          sizes={sizes}
          className="scale-[0.82] rotate-6 rounded-lg object-cover opacity-60 ring-1 ring-white/10"
      ...
      {thumbs.length > 1 && (
        <Image
          ...
          fill
          sizes={sizes}
          className="scale-[0.88] -rotate-3 rounded-lg object-cover opacity-80 ring-1 ring-white/10"
      ...
      {thumbs.length > 0 ? (
        <Image
          ...
          fill
          sizes={sizes}
          className="scale-95 rounded-lg object-cover shadow-[0_4px_10px_rgba(0,0,0,0.4)] ring-1 ring-white/15"
        />
      ) : (
        // 빈 폴더 — 아무것도 안 쌓인 판
        <div className="absolute inset-0 scale-95 rounded-lg bg-neutral-900 ring-1 ring-white/10" />
      )}
    </div>
  );
}
