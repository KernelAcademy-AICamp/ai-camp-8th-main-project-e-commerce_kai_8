...
// 원본: 물이 차오르는 연출(예전 `FeedSkeleton`) 없이 단순히 깜빡인다(`animate-pulse`).
// 실제 카드와 같은 2열 masonry 리듬으로 높이만 다르게 둔다.
//
// 홈 피드(브라우즈·검색)의 첫 로딩과 상세 하단 "이어보기" 모두 이 뼈대를
// 쓴다(2026-08-25 — 예전엔 상세만 물 차오름 연출을 썼다, 화면마다 뼈대가
// 갈려 있던 것을 통일했다). 무한 스크롤로 다음 배치를 받는 동안의 뼈대는
// `FeedGrid`의 `trailingSkeletonHeights`로 각 칸 끝에 직접 이어 붙인다 —
// 이 컴포넌트처럼 그리드 아래 별도 줄로 붙이면 칸마다 다른 실제 높이
// (masonry)가 반영되지 않는다(2026-08-24).
