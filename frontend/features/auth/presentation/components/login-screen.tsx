...
export function LoginScreen() {
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col px-6 pb-10 text-ink">
      <header className="-mx-2 flex items-center py-2">
        <button
          type="button"
          aria-label="닫기"
          onClick={close}
          className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full text-xl text-ink"
        >
          ✕
        </button>
      </header>
      ...
      {ready && (
        <div className="flex flex-1 flex-col justify-center gap-8 pb-16">
          <div className="space-y-3">
            <h1 className="text-2xl font-semibold text-ink">찜을 계정에 저장하세요</h1>
            <p className="text-[15px] leading-relaxed text-ink-soft">
              로그인하면 찜한 상품이 계정에 저장돼{" "}
              <span className="text-ink">다른 기기에서도</span> 보입니다.
            </p>
          </div>
          ...
          <div className="space-y-3 text-sm text-ink-muted">
            <p>
              ...
              <span className="text-ink-soft">함께 올라옵니다.</span>
            </p>
            <p>
              계속하면{" "}
